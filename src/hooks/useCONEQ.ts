...
export const useCONEQ = (gruposDeMunicipios: Record<string, string[]>) => {
  ...
  const removeAcentos = (texto: string): string => {
  ...
  const combineCautelasAndEntregas = (
    ...
    // Create dictionaries with normalized keys (uppercase and no accents)
    ...
    const dadosCombinados = cidadesSelecionadas.map(cidade => {
      const cidadeNormalizada = removeAcentos(cidade.toUpperCase());
      const dadoCidade = {
        nome_cidade: cidade,
  ...
  const handleGroupClick = async (crSelecionado: string[]) => {
    try {
      ...
      const cidadesQuery = cidadesMaiusculas.join(',');
      ...
      const combinedData = combineCautelasAndEntregas(cautelasData, entregasData, cidadesMaiusculas);
